...
import { clientDocumentService } from "../../../services/ClientDocument";
...
const SalesUploadClientDocument: React.FC = () => {
  ...
  const canSubmit = useMemo(() => {
    return !!clientContractPaymentId && !!documentTypeId && !!file && !submitting;
  }, [clientContractPaymentId, documentTypeId, file, submitting]);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file || !clientContractPaymentId || !documentTypeId) return;
    setSubmitting(true);
    setError(null);
    setSuccess(false);
    try {
  ...
  return (
    <div className="flex bg-gray-50 min-h-screen">
      <Sidebar items={salesSidebarItems} title="Sales Staff" />
      <div className="flex-1 p-8">
        ...
        <div className="mb-8 animate-slide-up">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Upload tài liệu Khách hàng</h1>
            <p className="text-neutral-600">Work report, Invoice, Bill ... gắn với kỳ thanh toán hợp đồng</p>
          </div>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-gray-700 font-semibold mb-2 flex items-center gap-2">
                    ...
                    ClientContractPaymentId <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
...
export default SalesUploadClientDocument;
