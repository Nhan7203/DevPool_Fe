...
export default function WorkingStyleListPage() {
  ...
  const stats = [
    {
      title: 'Tổng Phong Cách',
      value: workingStyles.length.toString(),
      color: 'blue',
    ...
    {
      title: 'Có Mô Tả',
      value: workingStyles.filter(ws => ws.description && ws.description.trim()).length.toString(),
      color: 'purple',
      icon: <FileText className="w-6 h-6" />
    },
    {
      title: 'Tỷ Lệ Có Mô Tả',
      value: `${Math.round((workingStyles.filter(ws => ws.description && ws.description.trim()).length / Math.max(workingStyles.length, 1)) * 100)}%`,
      color: 'orange',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await workingStyleService.getAll();
        setWorkingStyles(data);
        setFilteredWorkingStyles(data);
      } catch (err) {
        console.error("❌ Lỗi khi tải danh sách WorkingStyles:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const filtered = workingStyles.filter((ws) =>
      ...
      <div className="flex-1 p-8">
        ...
        <div className="mb-8 animate-slide-up">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Phong cách làm việc (Working Styles)</h1>
              <p className="text-neutral-600 mt-1">Quản lý và theo dõi các phong cách làm việc trong hệ thống</p>
            </div>
            <Link to="/admin/categories/working-styles/create">
          ...
          <div className="p-6">
            <div className="flex flex-wrap items-center gap-4">
              <div className="relative flex-1 min-w-[300px]">
                ...
                <input
                  type="text"
                  placeholder="Tìm kiếm theo tên phong cách làm việc..."
                  ...
                  <tr>
                    <td colSpan={4} className="text-center py-12">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-neutral-500 text-lg font-medium">Không có phong cách làm việc nào</p>
                        <p className="text-neutral-400 text-sm mt-1">Thử thay đổi từ khóa tìm kiếm hoặc tạo phong cách làm việc mới</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  filteredWorkingStyles.map((ws, i) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 text-center">
                        <Link
                          to={`/admin/categories/working-styles/${ws.id}`}
