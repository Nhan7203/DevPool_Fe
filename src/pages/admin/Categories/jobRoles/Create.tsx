...
import { type JobRoleCreatePayload, jobRoleService } from "../../../../services/JobRole";

export default function JobRoleCreatePage() {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState(false);
  const [error, setError] = useState("");

  const [form, setForm] = useState<JobRoleCreatePayload>({
    name: "",
    description: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await jobRoleService.create(form);
      setSuccess(true);
        ...
        <div className="mb-8 animate-slide-up">
          ...
          <div className="flex justify-between items-start">
            <div className="flex-1">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Tạo loại vị trí mới</h1>
              <p className="text-neutral-600 mb-4">
                Định nghĩa loại vị trí tuyển dụng trong hệ thống DevPool
              </p>
              ...
              {success && (
                <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
                  ...
                  <p className="text-green-700 font-medium">
                    ✅ Tạo loại vị trí thành công! Đang chuyển hướng...
                  </p>
                </div>
              )}
            </div>
