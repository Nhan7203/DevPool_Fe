...
import { ArrowLeft, Building2, Calendar, DollarSign, Clock, CheckCircle, AlertCircle, FileText, CreditCard, Download, Eye, Upload, FileUp, X, XCircle } from 'lucide-react';
import Sidebar from '../../../../components/common/Sidebar';
import { sidebarItems } from '../../../../components/manager/SidebarItems';
import { clientContractPaymentService, type ClientContractPayment, type ApproveForInvoicingModel, type RejectModel, type RecordInvoiceModel } from '../../../../services/ClientContractPayment';
...
export default function ClientContractDetailPage() {
    ...
    // Invoice info state (cho recordInvoice)
    const [invoiceNumber, setInvoiceNumber] = useState<string>('');
    const [invoiceDate, setInvoiceDate] = useState<string>('');
    const [invoicedAmount, setInvoicedAmount] = useState<number>(0);
    const [invoiceNotes, setInvoiceNotes] = useState<string>('');
    
    // Document state cho duyệt (Invoice - bắt buộc)
    const [invoiceFile, setInvoiceFile] = useState<File | null>(null);
    const [invoiceUploadProgress, setInvoiceUploadProgress] = useState(0);
    const [invoiceDocumentFormData, setInvoiceDocumentFormData] = useState<Partial<ClientDocumentCreate>>({
        documentTypeId: 0,
        fileName: "",
        filePath: "",
        description: "",
    ...
    const getStatusColor = (status: string): string => {
        const statusMap: Record<string, string> = {
            ...
            'ReadyForInvoice': 'bg-blue-50 text-blue-800 border-blue-200',
    ...
    const getStatusText = (status: string): string => {
        const statusMap: Record<string, string> = {
            ...
            'ReadyForInvoice': 'Sẵn sàng xuất hóa đơn',
    ...
    // Helper function để kiểm tra status có phải ReadyForInvoice không
    const isReadyForInvoice = (status: string | null | undefined): boolean => {
        if (!status) return false;
        const normalizedStatus = status.trim();
        // So sánh chính xác và case-insensitive để đảm bảo
        return normalizedStatus === 'ReadyForInvoice' || 
               normalizedStatus.toLowerCase() === 'readyforinvoice' ||
               normalizedStatus === 'Ready For Invoice' ||
               normalizedStatus.toLowerCase() === 'ready for invoice';
    };
    ...
    const handleOpenApproveModal = async () => {
        ...
        setInvoiceNumber('');
        setInvoiceDate('');
        setInvoicedAmount(payment?.calculatedAmount || 0);
        setInvoiceNotes('');
    ...
    // Hàm duyệt (bắt buộc phải có Invoice document)
    const handleApprove = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!payment) return;

        // Validate: Invoice document là bắt buộc
        ...
        // Validate: Thông tin hóa đơn là bắt buộc
        if (!invoiceNumber.trim() || !invoiceDate || invoicedAmount <= 0) {
            setApproveError('Vui lòng nhập đầy đủ thông tin hóa đơn (Số hóa đơn, Ngày hóa đơn, Số tiền)');
            return;
        }
        ...
        try {
            ...
            try {
                ...
                const path = `client-documents/${payment.id}/${Date.now()}_${invoiceFile.name}`;
                ...
                const payload: ClientDocumentCreate = {
                    ...
                    referencedPartnerDocumentId: invoiceDocumentFormData.referencedPartnerDocumentId || null,
            ...
            // Bước 2: Duyệt để xuất hóa đơn
            const approvePayload: ApproveForInvoicingModel = {
                paymentIds: [payment.id],
                notes: approveNotes || null
            };
            
            await clientContractPaymentService.approveForInvoicing(approvePayload);

            // Bước 3: Ghi nhận hóa đơn (recordInvoice)
            if (!invoiceNumber.trim() || !invoiceDate || invoicedAmount <= 0) {
                throw new Error('Vui lòng nhập đầy đủ thông tin hóa đơn (Số hóa đơn, Ngày hóa đơn, Số tiền)');
            }

            const recordInvoicePayload: RecordInvoiceModel = {
            ...
            await clientContractPaymentService.recordInvoice(payment.id, recordInvoicePayload);
    ...
    const handleReject = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                const allDocs = await clientDocumentService.getAll({
                    clientContractPaymentId: payment.id,
                    excludeDeleted: true
                });
                ...
                const accountantDocs = docs.filter((doc: ClientDocument) => 
            ...
            const rejectPayload: RejectModel = {
                rejectionReason: rejectionReason.trim()
            };
            
            await clientContractPaymentService.reject(payment.id, rejectPayload);
            ...
            <div className="flex bg-gray-50 min-h-screen">
                ...
                <div className="flex-1 flex justify-center items-center">
                    <div className="text-center">
                        ...
                        <Link
                            to="/manager/payment-periods/clients"
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white rounded-2xl shadow-soft p-6 border border-gray-100 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-xl font-semibold text-gray-900">Duyệt xuất hóa đơn và tạo tài liệu Invoice</h2>
                                <button
                            ...
                            <form onSubmit={handleApprove} className="space-y-6">
                                ...
                                {/* Thông tin hóa đơn (cho recordInvoice) */}
                                <div className="space-y-4 border-b pb-6">
                                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Thông tin hóa đơn <span className="text-red-500">*</span></h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => setInvoicedAmount(Number(e.target.value) || 0)}
                                                className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:border-primary-500 focus:ring-primary-500"
                                ...
                                {/* Tạo tài liệu Invoice - Bắt buộc */}
                                <div className="space-y-4">
                                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Tạo tài liệu Invoice <span className="text-red-500">*</span> (Bắt buộc)</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                            ...
                                            <select
                                                name="documentTypeId"
                                                value={invoiceDocumentFormData.documentTypeId || 0}
                                                onChange={handleInvoiceDocumentFormChange}
                                                className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:border-primary-500 focus:ring-primary-500"
                                                disabled={loadingInvoiceDocumentTypes}
                                                required
                                            >
                                ...
                                <div className="flex items-center justify-end gap-3 pt-4 border-t">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={approving || !invoiceFile || !invoiceDocumentFormData.documentTypeId || !invoiceNumber.trim() || !invoiceDate || invoicedAmount <= 0}
                            ...
                            <form onSubmit={handleReject} className="space-y-6">
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        required
                                    />
                                    <p className="text-xs text-gray-500 mt-1">Vui lòng nêu rõ lý do từ chối xuất hóa đơn</p>
                                </div>
